...
class BRAMSlave(depth: Int)(implicit val p: Parameters) extends Module
  ...
  val bram_data = Wire(UInt(width = tlDataBits)) // raw read-port output
  ...
  when (ren) {
    bram_data := bram.read(acq_addr)
  }

  // Implement bus-hold logic for the read data
  def holdUnless[T <: Data](in : T, enable: Bool): T = Mux(enable, in, RegEnable(in, enable))
  io.grant.bits.data := holdUnless(bram_data, RegNext(ren))
}
